...
const DICCIONARIO: Record<string, { en: string; es: string }> = {
  ...
  'aviso.edgeCasesEnCards': { en: "In decomposed layouts (Cards/Swimlanes/Table and dense-flow Classic), edge cases are already integrated; the 'separate edge cases' toggle was ignored for those flows.", es: 'En los layouts descompuestos (Cards/Swimlanes/Table y Classic de flujos densos) los edge cases ya están integrados; el toggle de separarlos se ignoró en esos flujos.' },
  'aviso.descomposicionFallback': { en: "Flow '{flujo}' is dense but no shared hub was found; it was rendered as a single diagram.", es: "El flujo '{flujo}' es denso pero no se encontró un hub compartido; se generó como un solo diagrama." },
  'canvas.laneSinReingreso': { en: 'No re-entry — flow ends here', es: 'Sin reingreso — termina el flujo' },
  'canvas.thTrigger': { en: 'Trigger', es: 'Disparador' },
  'canvas.thSistema': { en: 'What the system does', es: 'Qué hace el sistema' },
  'canvas.thResultado': { en: 'Outcome', es: 'Resultado' },
  'canvas.thReingresa': { en: 'Re-enters at', es: 'Reingresa a' },
  'canvas.leyendaConectores': { en: 'Connectors', es: 'Conectores' },
  'leyenda.conNormal': { en: 'Normal connector — flow sequence (#9747FF)', es: 'Conector normal — secuencia del flujo (#9747FF)' },
  'leyenda.conSalto': { en: 'Long jump / re-entry between distant parts (#0D99FF)', es: 'Salto largo / reingreso entre partes distantes (#0D99FF)' },
  'leyenda.conEdge': { en: 'Edge case / error — dashed line', es: 'Edge case / error — línea punteada' },
  'leyenda.conLabel': { en: 'Connector label badge', es: 'Badge de label del conector' },
